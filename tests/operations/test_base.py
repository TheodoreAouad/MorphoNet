...
def test_select(name, expected_class):
    result = Operation.select(name, structuring_element=MagicMock(), percentage=MagicMock())

    assert type(result) == expected_class

def test_select_neg():
    with pytest.raises(ValueError, match="The selected operation was not found"):
...
def test_empty_structuring_element():
    with pytest.raises(ValueError, match="Morphological operations need a structuring element."):
