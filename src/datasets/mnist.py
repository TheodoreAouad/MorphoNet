...
class MNIST(DataModule):
    def setup(self, stage: Optional[str] = None):
        train_dataset = torchvision.datasets.MNIST(
            self.dataset_path, train=True,
        )
        input_transformed = self.input_transform(train_dataset.data)
        self.train_dataset = Dataset(
            inputs=input_transformed,
            targets=self.target_transform(input_transformed, train_dataset.targets)
        )
        ...
        self.val_dataset = Dataset(
            inputs=input_transformed,
            targets=self.target_transform(input_transformed, val_dataset.targets)
        )
